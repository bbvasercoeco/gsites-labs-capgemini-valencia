...
function install(e) {
  ...
  if( found )
    panel.add(
      app.createCheckBox(repl_(T.overwrite, NAME)).setName('overwrite').setId('overwrite').setValue(overwrite).
      addValueChangeHandler(app.createClientHandler().forTargets(button.setEnabled(overwrite)).setEnabled(false)).
...
function proceedInstall_(e) {
  ...
  c.cs.getRange(5, 1, instructions.length, 2).mergeAcross().setValues(prepare);
